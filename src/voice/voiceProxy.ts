...
import { badgeAuthConfigured, isBadgeRequestAuthorized } from '../badge/badgeAuth';
...
export function attachVoiceWebSocket(server: Server): void {
  ...
  server.on('upgrade', (request, socket, head) => {
    ...
    if (isBadgeVoice && !badgeAuthConfigured()) {
      console.log('[voice] Badge connection rejected - badge authentication is not configured');
      socket.write('HTTP/1.1 503 Service Unavailable\r\n\r\n');
      socket.destroy();
      return;
    }
    if (isBadgeVoice && !isBadgeRequestAuthorized(request.headers)) {
      console.log('[voice] Badge connection rejected - invalid badge credential');
      socket.write('HTTP/1.1 401 Unauthorized\r\nWWW-Authenticate: Bearer realm="Morgan Badge"\r\n\r\n');
      socket.destroy();
      return;
    }
  ...
  wss.on('connection', async (clientWs, request) => {
    ...
    try {
      ...
      serviceWs.on('open', () => {
        ...
        // Configure session with Morgan's persona and HD voice. Badge sessions
        // are presentation-only and intentionally receive no callable tools.
        const sessionUpdate = {
          type: 'session.update',
          session: {
      ...
      serviceWs.on('message', async (data) => {
        ...
        if (isBadgeClient && event.type === 'response.audio.delta' && typeof event.delta === 'string') {
          if (clientWs.readyState === WebSocket.OPEN) {
            clientWs.send(Buffer.from(event.delta, 'base64'), { binary: true });
          }
          return;
        }

        // Forward all other events to the browser or badge.
        if (clientWs.readyState === WebSocket.OPEN) {
          clientWs.send(msg);
        }
      });
    ...
    clientWs.on('close', () => {
      console.log(`[voice] ${clientLabel} client disconnected`);
    ...
    clientWs.on('error', (err) => {
      console.error(`[voice] ${clientLabel} WebSocket error:`, err.message);
      if (serviceWs && serviceWs.readyState === WebSocket.OPEN) {
        serviceWs.close();
      }
    });
